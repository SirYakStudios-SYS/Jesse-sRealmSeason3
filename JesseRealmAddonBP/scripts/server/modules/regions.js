...
import { printDebug, regionSize, regionVolume, regionBounds } from '../util.js';
...
class RegionsManager {
    ...
    save(name, start, end, player, includeEntities = false, individualBlocks = false) {
            ...
            for (let z = 0; z < size.z; z += this.MAX_SIZE.z)
                for (let y = 0; y < size.y; y += this.MAX_SIZE.y)
                    for (let x = 0; x < size.x; x += this.MAX_SIZE.x) {
                        ...
                        const subEnd = Vector.min(new Vector(x, y, z).add(this.MAX_SIZE), size).add(min).sub(Vector.ONE);
    ...
    rotate(name, rotation, origin, player) {
        const structName = this.genName(name, player);
        const struct = this.structures.get(structName);
        if (struct) {
            const d = 360;
    ...
    flip(name, direction, origin, player) {
        const structName = this.genName(name, player);
        const struct = this.structures.get(structName);
        if (struct) {
